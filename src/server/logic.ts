// Fallback knowledge base for the Elections Assistant.
// Used when the Google Gemini API call fails or returns no content.
// Sources: Election Commission of India (ECI), ECINET, Tamil Nadu CEO website.
...
export function classifyQuery(query: string): FallbackKey {
  ...
  if (
    /\b(how.*vote|voting process|cast.*vote|polling.*procedure|at the booth)\b/.test(q)
  ) {
    return "how_to_vote";
  }
  if (
    /\b(first.?time|new voter|register|registration|enroll|18 year|epic apply)\b/.test(q)
  ) {
    return "first_time_voter";
  }
  if (
    /\b(document|id proof|identity|epic|aadhaar|what.*carry|what.*bring)\b/.test(q)
  ) {
